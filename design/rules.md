...
- Aoi action components use `variant` for visual form and `tone` for visual color. Low-emphasis `plain` + `muted` actions are the default; theme or semantic tones must be explicit. Feedback components such as status messages may keep `intent` when the color communicates information, success, warning, or danger.
